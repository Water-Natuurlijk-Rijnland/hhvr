  ...
  <div class="fixed top-20 right-4 bg-white rounded-lg shadow-2xl border border-gray-200 p-6 z-30 w-96 max-h-[calc(100vh-120px)] overflow-y-auto">
    ...
    <div v-else-if="chartData">
      <div class="mb-4">
        ...
        <p class="text-xs text-gray-500 mt-1">Laatste 24 uur</p>
      </div>
...
const fetchGemaalData = async (code) => {
  try {
    ...
    filterLast24Hours(chartData.value)
    loading.value = false
  } catch (e) {
...
// Filter data van laatste 24 uur
const filterLast24Hours = (data) => {
  ...
  // Filter alleen data punten van de laatste 24 uur
  ...
  const filtered = allDataPoints.filter(point => {
    ...
    return pointTime >= twentyFourHoursAgo && pointTime <= now
  })
  ...
  if (filtered.length > 0) {
    ...
    if (allDataPoints.length > 0) {
      ...
      if (hoursAgo > 24) {
        error.value = `Geen data beschikbaar voor laatste 24 uur. Meest recente data is van ${hoursAgo} uur geleden (${newestTime.toLocaleString('nl-NL')}).`
      } else {
        error.value = 'Geen data beschikbaar voor laatste 24 uur'
      }
    } else {
      error.value = 'Geen data beschikbaar voor dit gemaal'
    }
