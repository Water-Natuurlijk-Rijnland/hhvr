...
const layers = reactive({
        peilgebieden: {
          ...
          count: 0,
          color: '#ff7800',
          url: `${import.meta.env.BASE_URL}../data/kaartlagen/Peilgebied_vigerend_besluit/PeilgebiedVigerend_layer0.geojson`
        },
  gemalen: {
    visible: true,
    layer: null,
  ...
  try {
    ...
    if (data.features[0]?.geometry.type === 'Point') {
      geoJsonOptions.pointToLayer = (feature, latlng) => {
        ...
        // Specifieke logica voor gemalen
        if (layerName === 'gemalen' && feature.properties && feature.properties.CODE) {
           const status = gemaalStatussen.value[feature.properties.CODE]
           if (status && status.status === 'aan') {
             color = '#2563eb' // Blue-600
             shadow = 'text-shadow: 0 0 10px #3b82f6;'
           }
        }
        
        const iconDiv = L.divIcon({
          html: `<div style="font-size: 24px; color: ${color}; ${shadow}">${config.icon}</div>`,
          className: 'custom-icon',
          iconSize: [30, 30],
          iconAnchor: [15, 15]
        })
...
// Global gemaal status state
const gemaalStatussen = ref({})

const fetchGemaalStatussen = async () => {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}data/gemaal_status_latest.json`)
    if (response.ok) {
      const data = await response.json()
      if (data.stations) {
    ...
    console.error('Kon gemaal status niet laden:', e)
  }
}
  ...
  if (layers.gemalen.layer) {
    layers.gemalen.layer.eachLayer(layer => {
      if (layer instanceof L.Marker && layer.feature && layer.feature.properties) {
        ...
        const emoji = layers.gemalen.icon
        const color = isActive ? '#2563eb' : '#374151' // Blue vs Gray
        const shadow = isActive ? 'text-shadow: 0 0 10px #3b82f6;' : ''
        
        const newIcon = L.divIcon({
          html: `<div style="font-size: 24px; color: ${color}; ${shadow}">${emoji}</div>`,
          className: 'custom-icon',
          iconSize: [30, 30],
          iconAnchor: [15, 15]
        })
